...
#include "../print.h"
template<cp::integer n>
std::ostream & operator<<(std::ostream & os,const cp::cyclic<n>& v)
{
    return os << (cp::integer&)v;
}
...
BOOST_AUTO_TEST_SUITE(test_vector)
    BOOST_AUTO_TEST_SUITE(test_vector_constructor)
        ...
        {
            cp::linalg::vector<T, 5> v1({1, 2, 3, 4, 5});
    ...
    BOOST_AUTO_TEST_SUITE(test_vector_operator)
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_add_vector,T,test_types) {
            cp::linalg::vector<T,5> v1({1,2,3,4,5});
            cp::linalg::vector<T,5> v2({3,7,-1,-2,2});
            cp::linalg::vector<T,5> v3({4,9,2,2,7});
        ...
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_diff_vector,T,test_types) {
            cp::linalg::vector<T,5> v1({1,2,3,4,5});
            cp::linalg::vector<T,5> v2({3,7,-1,-2,2});
            cp::linalg::vector<T,5> v3({-2,-5,4,6,3});
            BOOST_CHECK_EQUAL(v1-v2,v3);
        }
        ...
        {
            cp::linalg::vector<T,5> v1({1,2,3,4,5});
            cp::linalg::vector<T,5> v2({3,6,9,12,15});
        ...
        {
            cp::linalg::vector<T,5> v1({1,2,3,4,5});
            cp::linalg::vector<T,5> v2({-1,-2,-3,-4,-5});
        ...
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_vector_self_add,T,test_types)
        {
            cp::linalg::vector<T,5> v1({1,2,3,4,5});
            cp::linalg::vector<T,5> v2({-1,-2,-3,-4,-5});
            cp::linalg::vector<T,5>&v3=v1+=v2;
            cp::linalg::vector<T,5> v4;
...
BOOST_AUTO_TEST_SUITE(test_matrix)
    BOOST_AUTO_TEST_SUITE(test_matrix_constructor)
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_create,T,test_types){
            cp::linalg::matrix<T,2,2> m1({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> m2({{1,2},{3,4}});

            BOOST_CHECK_EQUAL(m1[0][0],1);
            BOOST_CHECK_EQUAL(m1[0][1],2);
            BOOST_CHECK_EQUAL(m1[1][0],3);
            BOOST_CHECK_EQUAL(m1[1][1],4);
        }
    BOOST_AUTO_TEST_SUITE_END()
    ...
    BOOST_AUTO_TEST_SUITE(test_matrix_operator)
        ...
        cp::linalg::matrix<T,2,2> M1({{1,2},{3,4}});
        cp::linalg::matrix<T,2,2> M2({{3,7},{-1,-2}});
        cp::linalg::matrix<T,2,2> M3({{4,9},{2,2}});
        BOOST_CHECK_EQUAL(M1+M2, M3);
    }
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_diff,T,test_types) {
            cp::linalg::matrix<T,2,2> M1({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> M2({{3,7},{-1,-2}});
            cp::linalg::matrix<T,2,2> M3({{-2,-5},{4,6}});
            BOOST_CHECK_EQUAL(M1-M2, M3);
        }

        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_scalar,T,test_types)
        {
            cp::linalg::matrix<T,2,2> M1({{1,2},{3,4}});        cp::linalg::matrix<int,2,2> m1({{1,2},{3,4}});

            cp::linalg::matrix<T,2,2> M2({{3,6},{9,12}});
        ...
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_mult,T,test_types)
        {
            cp::linalg::matrix<T,2,2> M1({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> M2({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> M3({{7,10},{15,22}});
            BOOST_CHECK_EQUAL(M1*M2,M3);
        }
        ...
        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_self_mult,T,test_types)
        {
            cp::linalg::matrix<T,2,2> M1({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> M2({{1,2},{3,4}});
            cp::linalg::matrix<T,2,2> &M3=M1*=M2;
    ...
    {
        cp::linalg::matrix<T,4,4> A({{1,2,3,4},{3,1,2,4},{1,4,3,1},{5,3,1,2}});
        cp::linalg::matrix<T,4,4> B({{271373769, 257707107, 229627054, 291157093},
    ...
    BOOST_AUTO_TEST_SUITE(test_matrix_linalg)
        ...
        {
            cp::linalg::matrix<T,3,4> A({{1,2,3,4},{3,1,2,4},{1,4,3,1}});
            cp::linalg::vector<T,4> u({2,1,3,1});
            cp::linalg::vector<T,3> v({17,17,16});
            BOOST_CHECK_EQUAL(A*u,v);
        }
        BOOST_AUTO_TEST_CASE(test_matrix_det)
        {
            cp::linalg::matrix<IF,4,4> A({{1,2,3,4},{3,1,2,4},{1,4,3,1},{5,3,1,2}});
            BOOST_CHECK_EQUAL(A.det(),35);
        }

        BOOST_AUTO_TEST_CASE_TEMPLATE(test_matrix_trace,T,test_types)
        {
            cp::linalg::matrix<T,4,4> A({{1,2,3,4},{3,1,2,4},{1,4,3,1},{5,3,1,2}});
            BOOST_CHECK_EQUAL(A.tr(),7);
        }
