...
BOOST_FIXTURE_TEST_SUITE(test_ml,iris_fixture)
    BOOST_AUTO_TEST_SUITE(iris_dataset)
        ...
        {
            cp::linalg::matrix<real> X(data);
            std::vector<real> y(flower_class);
            cp::ml::multilogistic_regression M(C);
            if(C<30)
        ...
        using norm_types=boost::mp11::mp_list<
            cp::topology::L2_inner_product<real, cp::linalg::vector<real>>,
            cp::topology::L1_norm<cp::linalg::vector<real>>,
            cp::topology::L_inf_norm<cp::linalg::vector<real>>>;
        ...
        BOOST_AUTO_TEST_CASE_TEMPLATE(knearest_neighbours,A,KNN_types)
        {
            ...
            cp::linalg::matrix<real> X(data);
            std::vector<real> y(flower_class);
            cp::ml::k_nearest_neighbour_classifier<Norm> K(K_t::value);
